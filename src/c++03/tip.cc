...
public:
  ...
  int data_make_mave(const std::string &pattern, int nave) {
    std::vector<std::string> vlist=get_vlist(pattern);
    for(size_t j=0; j<vlist.size(); j++) {
      if(check_data2(vlist[0],vlist[j])) return 1;
    }
    for(size_t j=0; j<vlist.size(); j++) {
      thl::MovAve<double> mave(_dat[vlist[j]].num, nave);
      thl::CFormat fmt;
      std::string tag = vlist[j]+"_m"+fmt("%d",nave);
      _dat[tag].type = Num;
...
// zero padding
  ...
  int add_commands(std::string buf) {// called in MacroTool::parse_vbuf()
	...
	printf("Usage: fit x y [lin|quad|exp|log|gaus|sin|circ] [(opt)]\n"
	       ...
	       "  sin  : y = c0 + c1*sin(2*PI*c3x) + c2*cos(2*PI*c3*x)\n"
	...
	printf("Usage: hplot v [(opt)]\n"
	       ...
	       "Generates v_hx (bin centers) and v_hy (nentries), then plots them.\n"
	       "Options:\n"
	       "  nb:N      number of bins\n"
	       "  xr:x0,x1  x‑range\n"
