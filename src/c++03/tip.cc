...
public:
 ...
 double str_to_val(const std::string &s, DataType &type) {
  ...
  int data_set_time(const std::string &v, const std::string &expr,
	...
	_dat[v].type = Num; _dat[v].clear();
	for(size_t j=0; j<_dat[tag].str.size(); j++) {
	  _dat[v].num.push_back(var.ts.str_to_num(_dat[tag].str[j],unit));
	}
      }
      if(_dat[tag].type==Num) {
	if(unit=="str") {
	  _dat[v].type = Str; _dat[v].clear();
	  for(size_t j=0; j<_dat[tag].num.size(); j++) {
	    _dat[v].str.push_back(var.ts.utime_to_str(_dat[tag].num[j],1));
	  }
	} else {
	  _dat[v].type = Num; _dat[v].clear();
	  for(size_t j=0; j<_dat[tag].num.size(); j++) {
	    _dat[v].num.push_back(var.ts.utime_to_num(_dat[tag].num[j],unit));
	  }
	}
      }
    }
  ...
  void find_tags(std::vector<std::string> &tags, const std::string &expr) {
	...
	if(_dat[sp(j)].type==Mesh) {
	  ...
	  bool exist=0;
	  for(size_t k=0; k<tags.size(); k++) {if(sp(j)==tags[k]) exist=1;}
	  if(!exist) tags.push_back(sp(j));
	}
      }
    }
  }
  ...
  int data_make_cut(const std::string vlist, const std::string &expr) {
    ...
    if(tags.size()>0) {
      int quiet=1<<2;
      for(size_t k=0; k<tags.size(); k++) {
	if(check_data1(tags[k],quiet) == 2) flag_s=1;
      }
      for(size_t k=0; k<tags.size(); k++) {
	if(check_data2(sp(0),tags[k],quiet) > 2) return 1;
      }
  ...
  int data_make_mave(const std::string vlist, int nave) {
    thl::StrSplit sp(vlist,",");
    for(size_t j=0; j<sp.size(); j++) {
      if(check_data2(sp(0),sp(j))) return 1;
    }
    for(size_t j=0; j<sp.size(); j++) {
      thl::MovAve<double> mave(_dat[sp(j)].num, nave);
      thl::CFormat fmt;
      std::string tag = sp(j)+"_m"+fmt("%d",nave);
      _dat[tag].type = Num;
  ...
  void check_data_file(const std::string &fname, Option &opt) {
	...
	if(opt.fs==" \t\n") {
      ...
      int n=0;
      for(size_t j=0; j<buf.size(); j++) {if(buf[j]=='/') n++;}
      if(n>1) {
  ...
  template <class T> class VectorCompare {
    const std::vector<T> &_v;
  public:
    VectorCompare(const std::vector<T> &v) : _v(v) {}
    bool operator() (const size_t j, const size_t k) {
      return _v[j] < _v[k];
    }
  };
  int data_make_sort(const std::string &v_str, const std::string &v) {
    ...
    if(_dat[v].type==Num) {
      std::sort(idx.begin(),idx.end(),
		VectorCompare<double>(_dat[v].num)
		);
    }
    if(_dat[v].type==Str) {
      std::sort(idx.begin(),idx.end(),
		VectorCompare<std::string>(_dat[v].str)
		);
    }
    thl::StrSplit sp(v_str,",");
    int quiet=1<<2;
    for(size_t j=0; j<sp.size(); j++) {
      int ret=check_data2(v,sp(j),quiet);
      if(ret > 0 && ret != 2) return 1;
    }    
    for(size_t j=0; j<sp.size(); j++) {
      std::string s2 = sp(j)+"_sort";
      if(_dat[sp(j)].type==Num) {
	_dat[s2].type=Num;
	_dat[s2].num.resize(idx.size());
	for(size_t k=0; k<idx.size(); k++) {
	  _dat[s2].num[k]=_dat[sp(j)].num[idx[k]];
	}
      }
      if(_dat[sp(j)].type==Str) {
	_dat[s2].type=Str;
	_dat[s2].str.resize(idx.size());
	for(size_t k=0; k<idx.size(); k++) {
	  _dat[s2].str[k]=_dat[sp(j)].str[idx[k]];
	}
      }
    }
    return 0;
  ...
  int hist_plot(std::string v, Option &opt) {
    ...
    for(size_t j=0; j<_dat[v].num.size(); j++) {hist.fill(_dat[v].num[j]);}
