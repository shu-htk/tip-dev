...
//-----------------------------------------------------
  ...
  private:
	...
	return strtod(_cp,(char**)&_cp);
      ...
      for(size_t j=0; j<_tag.size(); j++) {
	if(s.find(_tag[j])==0) {
	  _cp += _tag[j].size();
	  return _var_num[_tag[j]];
	}
      }
      _not_digit=1;
      return 0;
...
//-----------------------------------------------------
  ...
  private:
	...
	if(*_cp=='=') {
	  ...
	  if(_nc.size()>0) {
	    double ret2=0;
	    for(size_t j=0; j<_nc.size();j++) {ret2 = (ret2||(lval==_nc[j]));}
	    ret = (ret && ret2);
	  } else {
	    ret = (ret && (lval == rval));
	  }
...
//-----------------------------------------------------
  ...
  public:
    ...
    void clear(void) {
      for(size_t j=0; j<_tag.size(); j++) {_var_str.erase(_tag[j]);}
      _tag.clear();
    }
  ...
  private:
	...
	if(bc.size()>0) {
	  ...
	  for(size_t j=0; j<sp.size(); j++) {_sc.push_back(trim(sp(j)));}
	  return "{";
	} else {
	...
	if(n != s.npos) _cp += n;
	for(size_t j=0; j<_tag.size(); j++) {
	  if(s.find(_tag[j])==0) {return _var_str[_tag[j]];}
	}
	return s.substr(0,n);
      }
    }
...
//    printf("tag[%s] ope[%s] expr[%s]\n",tag.c_str(),ope.c_str(),expr.c_str());
	...
	for(size_t k=0; k<tags.size(); k++) erase(tags[k]);
      }
    }
...
//-----------------------------------------------------
  class Loop {
    bool _init,_pass,_err;
    size_t _end_idx;
    std::string _tag;     // name of loop variable
    StrSplit _item;  // items of foreach loop
    size_t _nitem;    // index number of items of foreach loop
    double _n1,_dn;   // do loop _n1:end-number, _dn:increment
  public:
    Loop(void) :_init(1),_pass(0),_err(0),_end_idx(0),_nitem(0),_n1(0),_dn(1) {
      _item.set_quot_to_skip_split('"');
    }
...
//-----------------------------------------------------
  class If {
    ...
    size_t _else;
  public:
    If(void) : _pass(0),_err(0),_else(0) {}
    bool pass(void) {return _pass;}
    bool err(void) {return _err;}
...
//-----------------------------------------------------
  ...
  public:
    Var var;
    MacroTool(void) : _break(0), _quit(0){}
...
//      for(size_t j=0; j<vbuf.size(); j++) {printf("%s\n",vbuf[j].c_str());}
      size_t nline=0; // index number of line
